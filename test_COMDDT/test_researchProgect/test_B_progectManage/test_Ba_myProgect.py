...
class Test_projectManage:
    ...
    myProjectList = read_excel(researchCatePath, "我的项目-项目数据")

    @allure.title("项目管理->我的项目")
    @allure.story("我加入的")
    @allure.severity(A3)
    @pytest.mark.parametrize("redata, assert12, hint12", myProjectList)
    def test_list(self, redata, assert12, hint12):
        ...
        assert_get(url=url, params=dict(data, **redata), cook=self.cook, hint=assert12)
    ...
    MyProjectApplySave = read_excel(researchCatePath, "我的项目-项目申请-基本信息保存")

    @allure.title("项目审核 提交一")
    @allure.story("我加入的")
    @allure.severity(A2)
    @pytest.mark.parametrize("redata, assert12, hint12", MyProjectApplySave)
    def test_apply_save(self, redata, assert12, hint12):
        ...
        assert_post(url, dict(data, **redata), self.cook, hint=assert12)
    ...
    def test_apply_save_screen(self, questionId):
        ...
        dataDict = self.get_projectId(["10"])
        yamldata = congyaml["临床访视计划"]["添加访视计划"]
        allure.attach(f"项目Id={dataDict['projectId']}")
        data = {
            "projectId": dataDict["projectId"][0],
            "operatorId": self.authUserId,
            ...
            "deadlineType": 1,
            "followupPlan": yamldata["followupPlan"] % questionId[0],
            "startUpPeriodRule": 1,
    ...
    myProjectGroupSave = read_excel(researchCatePath, "我的项目-申请项目-受试者分组保存数据")

    @allure.title("我的项目->申请项目：受试者分组  保存数据")
    @allure.story("我加入的")
    @allure.severity(A2)
    @pytest.mark.parametrize("redata, assert12, hint12", myProjectGroupSave)
    def test_apply_save_groups(self, questionId, redata, assert12, hint12):
        ...
        assert_post(url, dict(data, **redata), self.cook, assert12)

    myProjectCenterList = read_excel(researchCatePath, "我的项目-未提交-分中心数据列表")

    # @allure.title("申请项目：分中心 数据展示")
    # @allure.story("我加入的")
    # @allure.severity(A3)
    # @pytest.mark.parametrize("redata, assert12, hint12", myProjectCenterList)
    # def test_apply__save_groupsList(self, redata, assert12, hint12):
    ...
    #         "authToken": self.authToken
    #     }
    #     assert_get(url, dict(data, **redata), self.cook, assert12)
    ...
    #         "projectGroupsData": '[{"projectCount":"5550","groupProjectId":7397}]',
    #         "centerCode": 5435,
    #         "authUserId": self.authUserId,
    #         "authToken": self.authToken
    #     }
    ...
    @allure.story("我加入的")
    @allure.severity(A3)
    def test_share_uploadingFileDelect(self):
    ...
    @allure.title("修改参加的人数")
    @allure.story("项目审核后的操作")
    @allure.severity(A2)
    def test_startStatus_updataUser(self):
        url = host + port_project + "/project/user/practitioner/update.json"
        ids = self.get_practitionerList()
        data = dict(id=ids,
                    projectQuota=500,
                    projectFunction="MANAGE_PATIENT,MANAGE_PRACTITIONER,CREATE_INFORMED_CONSENT,"
                                    "CHECK_INFORMED_CONSENT,CREATE_CRF,CHECK_CRF,CREATE_EVENT,CHECK_EVENT",
                    operatorId=self.authUserId, updateFunction="true",
                    authUserId=self.authUserId, authToken=self.authToken)
    ...
    @allure.story("项目审核后的操作")
    @allure.severity(A3)
    def test_transfer_saveBase(self):
        url = host + port_project + "/project/info/base.json"
        dataDict = self.get_projectId(["3"])
        allure.attach(f"项目Id={dataDict['projectId']}")
        data = {
        ...
        global projectGroupId
        projectGroupId = result[1]["responseData"]["startUpPeriodDTO"]["projectGroupId"]
        print(f"projectGroupId={projectGroupId}")
    ...
    @allure.story("项目审核后的操作")
    @allure.severity(A3)
    def test_project_patientList(self):
    ...
    @allure.title("保存CRF记录-需要填写的CRF记录")
    @allure.story("项目审核后的操作")
    @allure.severity(A3)
    ...
    def linkId(self, dlogin):
        ...
        data = dict(token=token, index=0,
                    isFirst=1, patientId=patientId[0],
                    authUserId=self.authUserId, authToken=self.authToken)
        linkId = "d3c2e54f-3162-40ab-84a7-7c38175666f3"
        result = requests.get(url, data, cookies=self.cook)
    ...
    @allure.title("用于CRF的提交")
    @allure.story("项目审核后的操作")
    @allure.severity(A2)
    def resourceId(self):
        url = host + port_project + "/project/user/practitioner/resourceId.json"
        data = {
            "operatorId": self.authUserId,
            "authUserId": self.authUserId, "authToken": self.authToken}
        result = assert_get(url, data, self.cook)
        practionerId = result[1]["responseData"]["practitionerId"]
        return practionerId
    ...
    def test_startStatus_qtCheck(self, dlogin):
        ...
        data = {
            ...
            "questionnaireUrl": "http://ys1hospital.com/fhir/Questionnaire/xzwj0011-1573805131063",
            "type": "submit",
            "authUserId": self.authUserId,
            "authToken": self.authToken
        }
    ...
    def test_startStatus_interviewSubmit(self, dlogin):
        ...
        linkId = self.linkId(dlogin)
        resdata = self.getcarePlanId_ID()
        practionerId = self.resourceId()
